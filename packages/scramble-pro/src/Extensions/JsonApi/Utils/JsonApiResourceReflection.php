...
namespace Dedoc\ScramblePro\Extensions\JsonApi\Utils;
...
class JsonApiResourceReflection
{
    use FlattensMergeValues;

    public function __construct(public readonly string $name, public readonly ClassDefinition $definition) {}
    ...
    public function getAttributesType(): ?KeyedArrayType
    {
        ...
        return new InferType\KeyedArrayType([
            ...$this->flattenMergeValues($propertiesAttributesType->items ?? []),
            ...$this->flattenMergeValues($toAttributesReturnType->items ?? []),
        ]);
    }
    ...
    private function getMethodReturnType(string $method): ?Type
    {
        ...
        return TypeHelper::unpackIfArray($array);
    }
